...
sql_discover_list = [
	"select code,standard_name,abbreviation,name_cn,full_name,alternative_name from yymf_discover_target "]
sql_discover_where = "where is_delete <> 1 and id <> 1 "
...
def writedict(isdiscover=True, iswritedict=True, isfuzzymatch=False, readcache=False, returntype=0):
	return PutLabelCore.writedict(MatchType.target, None, [sql + sql_discover_where for sql in sql_discover_list],
...
if __name__ == '__main__':
	dict_discover_target, jieba_dict_discover_target = writedict(isdiscover=True)
	print("over")
